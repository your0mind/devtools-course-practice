...
double Elasticity::PriceElasticity(double Q1,
    ...
    if (IsZero((P2 - P1)|| IsZero((Q2 + Q1)))) throw std::string("Can't divide by zero");
    coeff =
    fabs(((Q2 - Q1) / (Q2 + Q1)) *
    ((P2 + P1) / (P2 - P1)));
    return coeff;
...
double Elasticity::IncomeElasticity(double Q1,
    ...
    if (IsZero((Y2 - Y1) || IsZero((Q2 + Q1)))) throw std::string("Can't divide by zero");
     coeff =
    ((Q2 - Q1) / (Q2 + Q1)) *
    ((Y2 + Y1) / (Y2 - Y1));
    return coeff;
...
double Elasticity::CrossElasticity(double Q1X,
    ...
    if (IsZero((P2Y - P1Y) || IsZero((Q2X + Q1X)))) throw std::string("Can't divide by zero");
    coeff =
    ((Q2X - Q1X) / (Q2X + Q1X)) *
    ((P2Y + P1Y) / (P2Y - P1Y));
    return coeff;
